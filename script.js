                ...
                if (["id"].indexOf(j) < 0) // these are not displayed
                    if (["desiredDominance","domicoins"].indexOf(j) < 0) // these are displayed as pure numbers
                        if (j == "size") // this is a special case
                            content += "<div class='att'> "+(j.substr(0,2)+":").rpad(".",2)+(("## "+creatureList[k][j]).lpad("#",creatureList[k][j]-20+5).rpad(".",10))+"</div>";
                        else if (j == "skill")
                            content += "<div class='att'> "+(j.substr(0,2)+":").rpad(".",2)+("#".lpad("#",creatureList[k][j]).rpad(".",10))+"</div>";
                        else // these are bars
                            content += "<div class='att'> "+(j.substr(0,2)+":").rpad(".",2)+("#".lpad("#",creatureList[k][j]/10).rpad(".",10))+"</div>";
                    else
